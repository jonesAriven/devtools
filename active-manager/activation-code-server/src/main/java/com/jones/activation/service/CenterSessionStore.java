 ...
 * 「本系统用户」页需要以**中心身份**调 {@code /admin/**}，而 activecode 的会话是 HttpSession，
 * 浏览器侧没有中心凭据。两种做法：
 * <ol>
 *   <li>把中心 accessToken 回传前端存 localStorage，每次请求带上 —— 等于给页面加了一个
 *       可被 XSS 直接窃取的**中心管理凭据**，且它不受本应用登出控制；</li>
 *   <li>（本实现）服务端按用户名暂存，{@link com.jones.activation.controller.AdminProxyController}
 *       转发时取用 —— 与 infra-monitor {@code CenterSessionStore} / portal
 *       {@code AuthCenterService.refreshTokens} 同一模式，浏览器侧只留 JSESSIONID。</li>
 * </ol>
 *
 * <p><b>已知边界</b>（与 infra-monitor 相同）：内存态，服务重启即失效 → 账密会话访问
 * 「本系统用户」会返回 401 触发重授权；亦要求单实例部署（本应用为单容器）。
 * SSO 会话不受影响 —— 它带来的中心 OIDC access token 同样登记在此，重启后重新 SSO 即可。
 */
@Component

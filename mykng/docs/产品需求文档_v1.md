...
|------|--------|------|----------------------------|
| M01 | 用户登录/登出 | auth-center | Given 用户已注册且未登录，When 输入正确账号密码，Then 返回 accessToken（15分钟）+ refreshToken（7天），密码不明文返回 |
| M02 | Token 刷新 | auth-center | Given accessToken 已过期且 refreshToken 有效，When 调用刷新接口，Then 返回新的 accessToken 和 refreshToken，旧 refreshToken 失效（一次性） |
| M03 | 用户管理 | auth-center | Given 管理员已登录，When 创建/禁用/删除用户，Then 用户表对应记录变更，敏感字段（密码）脱敏返回 |
| M04 | API Token 管理 | auth-center | Given 用户已登录，When 创建 API Token，Then Token 加密存储，可用于脚本调用，支持吊销 |
...
| 编号 | 功能点 | 模块 | 验收标准 |
|------|--------|------|---------|
| S01 | 操作日志 | auth-center | 关键操作（登录/CRUD/导入）记录到 operation_log，含 traceId，支持按时间/用户/操作类型查询 |
...
| auth-center | 8085 | 认证授权、用户管理、API Token | AuthController, UserController, ApiTokenController |
...
- `api/` — API 请求封装（按服务分文件）
- `views/` — 页面（auth/file/knowledge/intelligence）
- `components/` — 可复用组件
- `router/` — 路由（含导航守卫）
- `stores/` — Pinia 状态管理
- `utils/` — 工具函数

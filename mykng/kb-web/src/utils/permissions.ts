...
import { CONTEXT_PATH, OIDC_CLIENT_ID, OIDC_ISSUER } from '@/config'
...
export function permCode(type: 'menu' | 'api', code: string): string {
  return `${OIDC_CLIENT_ID}:${type}:${code}`
}

/**
 * 本应用的权限语境（菜单 / 路由守卫 / PermissionGate **必须共用同一份**）。
 *
 * `getToken` 显式注入本应用的凭据读法：kb-web 的 token 由 `utils/token.ts` 管理，
 * 不是组件库默认键，不注入的话 `/auth/permissions` 请求会不带 Bearer → 401 → 永远未配置。
 */

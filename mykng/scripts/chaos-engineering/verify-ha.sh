...
health_check() {
    ...
    info "[$label] 健康检查..."
    local all_ok=true
    for svc in platform-mysql platform-redis platform-mongo platform-minio platform-meilisearch kb-gateway auth-center kb-file kb-knowledge kb-intelligence; do
        local status health
...
recover_all() {
    ...
    # 清除所有 tc 规则
    for svc in kb-gateway auth-center kb-file kb-knowledge kb-intelligence; do

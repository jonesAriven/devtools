/**
 * infra-monitor SSO（OIDC authorization_code + PKCE）—— **薄适配层**（Phase 6 组件收敛）
 *
 * 实现已全部收敛到公共组件 `@marschat/auth-components`；本文件只做两件事：
 * 1. 绑定 infra-monitor 自己的 OIDC 配置（client_id=marschat-inframon，回调 /infra/sso-callback）；
 * 2. 保持既有导出名与调用签名不变，上层（LoginView / SsoCallbackView / request）零改动。
 *
 * ⚠️ 本文件**不再**自己实现 PKCE / 换票 / 续期逻辑。此前 kb-web、kb-ops-web、
 * infra-monitor-web 各抄一份 150+ 行，改一处要改三处。
 *
 * 新增能力（组件提供，本文件透传）：
...
//   - request:   `renewByReauthorize(`${pathname}${search}`)`

 ...
 * 1. 绑定 infra-monitor 的 localStorage key（保持 `infra_` 前缀，不动老用户的登录态）；
...
initTokenConfig({
  accessTokenKey: 'infra_access_token',
  refreshTokenKey: 'infra_refresh_token',
  tokenKindKey: 'infra_token_kind',
  // 统一登出（SLO）要用 id_token 作 id_token_hint
  idTokenKey: 'infra_id_token',
})
